//sPHENIX Channel Deadmap Mar 17, 2023
...
void Livemap_Analyze( )
{
  ...
  TNtuple *h_myTntupleLive=new TNtuple("h_myTntupleLive","Location Tntuple","sector_id:fee_id:channel_id");
  TNtuple *h_myTntLive=new TNtuple("h_myTntLive","MW Location Tntuple","sector_id:fee_id:channel_id:pedestal:noise");
  // TNtuple *h_myTnttLive=new TNtuple("h_myTnttLive","MW-Full Data Location Tntuple","sector_id:fee_id:channel_id:pedestal:noise");
  ...
  for( int i = 0; i < 72; i++ )
  {
    ...
    Ana( h_myTntupleLive, h_myTntLive,  means_dist, widths_dist, run, tot, dead, i); //create Ana function
...
{ // Ana starts     
  ...
  for (int i=0;i<2304;i++){
    ...
    if (pedestal > 0 && pedestal > 110 || noise < 0.5) continue;
    ...
    //     std::cout << "sector " << sec_fee_id.first << "   fee " << sec_fee_id.second << "   channel " << i%256 << std::endl;

    ntup->Fill(1.0*sec_fee_id.first,1.0*sec_fee_id.second,1.0*(i%256));
    ntuptup->Fill(1.0*sec_fee_id.first,1.0*sec_fee_id.second,1.0*(i%256),1.0*pedestal,1.0*noise);
  }
 ...
 */
  //Already getting total from Deadmap_Analyze.C. Don't need to rerun here 
}
