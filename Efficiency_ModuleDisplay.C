...
// jennifer.l.james@vanderbilt.edu, chughes2@IASTATE.EDU // 
...
void Efficiency_ModuleDisplay(){
  ...
  bool skip_empty_fees =false;  // false will keep empty FEEs or FEEs that have 0 channel output
                                // true will skip these and have ONLY the ACTIVE FEEs

  std::vector<pair<int,int>> vec; 

  const TString filename3( Form( "./pedestal-10179-outfile.root") ); //change the run number accordingly
  ...
  liveTntuple->Draw("module_id:sec_id:fee_id>>h_AlivePedestalNoise","pedStdi",""); //x axis = FEE ID, y axis = SEC ID, z axis = MODULE ID 
  ...
  //sum_noise is the same functionality in Noise_ModuleDisplay.C
  ...
  for (Int_t i = 0; i < dm2->GetNbinsY(); i++) { // i is looping over sec ID                                                           
    ...
    for (Int_t j = 0; j < dm2->GetNbinsZ(); j++) { // j is looping over Module ID 
      ...
      if (i < 12) { // create 2 arrays from frac_val that split to make the North and South side of TPC
  ...
  ErrCSide->SetMinimum(30); //set the minimum frac_val limit here
